  ...
  <div class="container-xl">
    <div class="row justify-content-center">
      <div class="col-lg-6 col-md-8">
        <form class="card" method="POST" action="{{ route('pricing.update', $pricing->id) }}">
          @csrf
          @method('PUT')
          <div class="card-header">
            <h3 class="card-title">Edit Existing Plan</h3>
          </div>
          <div class="card-body">
            <div class="mb-3">
              <label for="name" class="form-label required">Name</label>
              <input id="name" name="name" type="text" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $pricing->name) }}">
              @error('name')<div class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></div>@enderror
            </div>
            <div class="mb-3">
              <label for="price" class="form-label required">Price</label>
              <input id="price" name="price" type="text" class="form-control @error('price') is-invalid @enderror" value="{{ old('price', $pricing->price) }}">
            ...
            <div class="mb-3">
              ...
              <input id="list" name="list" type="text" class="form-control @error('list') is-invalid @enderror" value="{{ old('list') ? collect(json_decode(old('list')))->pluck('value') : $pricing->list }}">
              @error('list')<div class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></div>@enderror
            </div>
            <div class="form-check mb-3">
              <input class="form-check-input" type="checkbox" value="true" id="recommended" name="recommended" {{ $pricing->recommended ? ' checked':'' }}>
