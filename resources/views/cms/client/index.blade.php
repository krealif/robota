        ...
        <div class="table-responsive">
          ...
          <table class="table table-vcenter table-mobile-md card-table">
            ...
            <tbody>
              ...
              <tr>
                <td>{{ $client->name }}</td>
                <td><img src="{{ $client->getImage() }}" height="48"></td>
                <td>
                  <div class="btn-list flex-nowrap">
                    <a href="{{ route('client.edit', $client->id) }}" class="btn">
                      Edit
                    </a>
                    <button id="delete" class="btn" data-bs-toggle="modal" data-bs-target="#modal-delete" onclick="deleteItem({{ $client->id }})">
                      Delete
                    </button>
                  </div>
                </td>
          ...
          @else
              <div class="card-body">There are no client logos yet.</div>
          @endif
        </div>
...
<script>
  function deleteItem(id) {
    ...
    deleteForm.action = '{{ route('client.index') }}/' + id;
  }
</script>
@endpush